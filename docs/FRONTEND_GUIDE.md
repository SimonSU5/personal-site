...
- framer-motion（详情视图动画）、react-markdown + remark-gfm + rehype-sanitize + **自研 remark-obsidian**（渲染博客/作品正文 + Obsidian 内链）、mermaid（markdown 里的流程图）、react-syntax-highlighter、lucide-react（图标）、sharp（上传图片预处理）
...
| POST | `/api/github/sync` | ⚠️ 无鉴权（建议补 `checkAuth`） | 拉内容仓库 `blogs/works/assets` → posts/works.json + 根 `assets/`；cover 与正文图片改写为本地 `/assets/...` |
...
- `data/` 与项目根 `assets/` 都已 `.gitignore`，其中的 json / 附件另设了 `--skip-worktree`：**仓库保留一份示例快照，本地编辑 / 重新同步的改动不入 git**。
- **`assets/`**（项目根，**非** `public/`）—— GitHub 同步下来的 Obsidian 附件（`covers/`、`images/` …），由 `app/assets/[...path]/route.ts` 提供静态服务。`posts`/`works` 的 `cover` 与正文图片都指向 `/assets/...`。
...
1. **syncAssets 先跑**：Git Trees API（`recursive=1`）列出 `assets/` 下所有 blob，再用 Blobs API（`git/blobs/{sha}`）取内容（绕开 Contents API 单文件 1MB 限制）→ 写项目根 `assets/<相对路径>`，并构建 `basename → 相对路径` 索引。逐文件 try/catch；assets 异常被外层兜住，不影响内容同步。
2. **syncPosts / syncWorks**：拉 `blogs/`、`works/` 的 `.md`，解析 frontmatter，用上面的索引把 `cover` 与正文 `![[图片]]` 改写为本地 `/assets/...`，落 `data/posts.json` / `data/works.json`。
